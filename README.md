...
#### JAVA - 프로그래밍 언어
	프로그래밍 언어는 개발자와 운영체제(OS)가 소통하기 위한 언어.

#### 소스코드
	운영체제와 개발자 간의 소통을 글로 적어놓은 것. 대화록

#### 소스파일(.java)
	소스코드를 작성해놓은 파일.

#### 컴파일
	개발자가 작성한 소스코드를 컴퓨터 언어로 바꿔주는 작업.

#### 컴파일러(번역기)
	컴파일 해주는 프로그램 혹은 명령어.
	컴파일러는 위에서 아래로, 좌에서 우로 해석한다.
...
#### JRE(Java Runtime Environment)
	JVM을 생성하며, 실행할 때 필요한 라이브러리를 제공한다.
...
#### 출력 메소드를 사용하는 이유
	...
	오류 발생 시, 어떤 라인에서 발생했는지 파악할 수 있다.
...
#### 자료형(type)
	...
	정수형	byte		1		1, 4, 1, -120, 100,...
		...
		long		8		30, 8, -312321789, ...

	실수형	float		4		3.12f, 237.41f, -53.6f...
		double		8		45.12, -35.124, 56.251...
...
#### 변수를 사용하는 이유
	1. 반복되는 값을 쉽게 관리하기 위해
	2. 의미 없는 값의 하나의 정보로 만들기 위해 (자료 구조)
...
#### 조건식
	...
	논리 연산자
		&& (AND), A && B => 두 조건이 모두 참이면 참
		|| (OR), A || B  => 두 조건 중 하나라도 참이면 참

	단항 연산자
		! (NOT), !A => 해당 조건이 참이면 거짓, 거짓이면 참
...
#### ▶ 조건문
	...
	※ 삼항 연산자, if 문, switch 문 비교
		삼항 연산자 : 조건식을 1개만 사용할 때
		if 문 : 조건식에 비교(>, <, >=, <=) 연산자를 사용하거나, 여러 개의 조건식을 논리 연산자로 연결할 때
		switch 문 : 하나의 변수에 여러 경우의 값이 담길 수 있고, 각 값이 같은지 비교할 때
...
#### ▶ 기타 제어문
	▷ break : 즉시 해당 { } 영역을 탈출한다.
			- if 문에서 사용 시, if문을 감싸고 있는 { } 영역을 탈출한다.

	▷ continue : 즉시 다음 반복으로 넘어간다.
			- 아래의 코드를 실행하지 않기 위해 사용한다.
...
#### 배열의 선언
	...
	자료형[] 배열명 = new 자료형[칸수]; // 어떤 값을 넣을지 모르나, 몇 칸 만들지는 알 때 사용한다.
...
#### 배열의 구조
	...
	즉, arData[2]는 1이란 값이 된다. 방 번호는 index라고 부르며, 인덱스 번호는 항상 0부터 시작된다.
...
#### 메소드 선언
	(1)리턴 타입 (2)메소드명 (자료형 (3)매개변수명, ...) {
		(4)실행할 문장;
		(5)return 리턴값;
	}

	(1) 리턴값이 있다면 리턴값의 자료형을 작성
	(2) 동사로 작성한다.
	(3) 외부에서 전달받은 값이 있다면, 자료형과 순서에 맞게 선언해준다.
...
#### 다형성(Polymorphism)
	...
	2. 오버라이딩(Overriding)
		...
		같은 이름의 메소드로 선언하며, 자식 객체로 접근했을 때 가까운 곳에서 찾기 때문에
...
#### 기본 생성자
	...
	사용자가 직접 생성자를 선언하게 되면 기본 생성자를 만들어주지 않는다.
...
#### super() : 부모 생성자
	...
	사실은 자식 생성자에서 항상 부모 생성자를 호출하게 되며, 자식 생성자 호출 시
	부모 필드와 자식 필드 모두 메모리에 할당된다.
	이때, 부모 생성자를 호출하는 방법은 super()를 사용하는 것이다.
	만약 super()를 작성하지 않아도 컴파일러가 자동으로 작성해준다.
	반드시 코드 맨 첫줄에 작성하고 시작한다.

#### 접근 권한 제어자(접근자)
	1. default : 다른 패키지에서 접근 불가.
	2. public : 모든 곳에서 접근 가능, 만약 클래스 앞에 붙는다면 해당 파일의 메인 클래스를 의미한다.
...
#### Casting
	...
	2. down casting
		up casting된 객체는 다시 자식 타입에 담을 수 있다.

	※ 부모 값은 자식 타입으로 형변환 시 빌드 오류 발생.
...
#### 인터페이스(틀)
	...
	구현은 지정한 클래스에서 진행하고, 인터페이스를 다른 클래스에 지정할 때
	implements 키워드를 사용한다.

#### 추상 클래스와 인터페이스 간의 관계
	인터페이스를 클래스에 바로 지정하면 모든 메소드에 강제성이 부여되기 때문에 전부 다 구현해야 한다.
	하지만 일반적인 상황에서는 필요한 메소드를 골라서 재정의한다. 이 때에는 인터페이스를 직접 지정하지 않고
	다른 클래스에 지정해서 구현해 놓는다면, 강제성이 소멸되고 이로 인해 골라서 재정의할 수 있게 된다.
	이 때 중간에서 강제성을 없애주는 클래스를 추상 클래스로 선언하기도 하며,
...
#### 인터페이스 특징
	...
	4. default 메소드를 사용하면, 인터페이스 내부에서 일반 메소드 선언이 가능하다.(JDK 8 버전부터)
...
#### 익명 클래스(Anonymous Inner Class)
	이름 없는 클래스이며, 구현되지 않은 필드를 구현하기 위해 일회성으로 생성되는 클래스이다.

-------------------------------------------------------------------------------------

#### 함수형 인터페이스(Functional Interface)
	인터페이스 중 추상 메소드를 하나만 가지고 있는 인터페이스를 '함수형 인터페이스'라고 한다.
	이때 @FunctionalInterface를 인터페이스 위에 작성하여 단 하나의 추상 메소드만 선언하도록 제한한다.

#### 람다식(Lambda Expression)
	함수형 인터페이스에서는 한 개의 추상 메소드만 존재하기 때문에 구현 시 메소드 이름이 딱히 필요가 없다.
	람다식은 이름이 없는 메소드로써, 값처럼 사용이 가능하고 매개변수로도 전달이 가능하다.
...
#### 예외 처리 문법
	...
	...
		...
		catch에서 잡히지 않은 오류가 있더라도 무조건 실행되는 문장;
	}
...
#### throws
	...
	하나의 메소드 내에 같은 예외가 반복해서 발생하면 try~catch문을 사용하는게 번거롭다.
	따라서 throws를 작성한 뒤 메소드를 사용한 부분에서 딱 한번 try~catch문을 사용하는 것이 낫다.
...
#### Object 클래스
	...
	1. toString()
		...
		따라서 객체명만 출력 메소드에 전달하더라도 toString()의 리턴값(문자열 값)이 출력된다.
		기본적으로 Object에 선언된 toString()은 소속된 필드 주소를 문자열로 리턴해준다.
	...
	2. equals()
		...
		만약 주소가 아닌 원하는 필드를 비교하고자 한다면 반드시 equals()를 재정의하여 사용한다.

	3. hashCode()
		JAVA에서 사용하는 고유한 주소값이다.
		JVM에서 관리하는 중복없는 값으로서 실제 메모리 주소와는 다르다.
...
##### 1. List<>
	...
	※ 배열과 ArrayList의 차이
		배열은 길이의 제한을 두어야할 때 사용.
		ArrayList는 몇개의 데이터가 들어올지 알 수 없을 때 사용된다.

##### 2. Set<> (집합)
	Set은 인터페이스이고 List와 마찬가지로 Collection 인터페이스를 상속받았다.

	HashSet
		전달받은 값의 hashCode를 비교하여 같다면 무시한다.
		저장된 값들은 인덱스가 없기 때문에 순서가 없다.
		값의 유무 검사에 특화되어 있는 자료구조이고 해시코드로 유무 검사가 진행되어
		속도가 상대적으로 좋다.
...
##### 3. Map<> (서버 간 데이터 전달)
	...
	List와 Set 처럼 Collection을 상속받지 않았고, 독립적인 자료구조이다.
...
쓰레드
	...
	- 싱글 쓰레드(단일 쓰레드)
		...
		하나의 작업에 문제가 발생하더라도 다른 작업에는 문제가 발생하지 않는다. 따라서 안정성이 보장된다.
		또한 멀티 쓰레드에 비해 설계가 쉽다.

	- 멀티 쓰레드(다중 쓰레드)
		하나의 프로세스를 동시에 처리하는 것처럼 보이지만 사실은 매우 짧은 단위로 분할해서 차례로 처리한다.
		여러 개의 처리 경로를 가질 수 있도록 하며, 동시 작업이 가능해진다.
		하나의 쓰레드 문제 발생 시, 모든 쓰레드에 문제가 발생하게 되므로 단점을 감수하고 설계하는 편이다.
		처리량 증가, 효율성 증가, 생산성 증가의 장점이 있다.
...
멀티 쓰레드 구현 방법
	...
	1. Thread 클래스 상속
		...
		해당 객체에서 부모(Thread)의 start()를 사용하여 스케줄링 한다.

	2. Runnable 인터페이스 구현
		run() 메소드를 구현하여 자원을 구성하고
		해당 객체를 Thread 생성자에 전달하여 Thread 객체로 start()를 사용한다.

	※ extends는 단 한번 사용할 수 있기 때문에, 보통 Runnable을 구현하여 멀티 쓰레드 환경을 구축한다.
	이 때, Runnable은 함수형 인터페이스이기 때문에 run() 메소드를 람다로 구현하여 사용할 수 있다.

#### 동기화(Synchronized)
	하나의 쓰레드가 자원에 접근 중일 때 다른 쓰레드가 동시에 같은 자원을 접근하지 못하게 막는 기술이다.
	자원 공유 시, 각 쓰레드가 접근하는 자원을 각자의 캐시에서 관리하기 때문에
	결과값이 기획대로 반영되지 않는 경우가 생긴다. 이를 자원 공유 문제라고 부르고, 동기화로 이 문제를 해결할 수 있다.
...
#### String
	...
	만약 + 연산으로 문자열을 반복해서 연결하게 되면 그만큼의 객체가 생성되기 때문에
	메모리 비용을 많이 발생시킨다.
...
#### StringBuffer
	...
	StringBuilder(JDK 5부터 등장)가 없었을 때 StringBuffer만 존재했으므로
	오래된 코드나 라이브러리들은 여전히 StringBuffer를 사용하고 있을 수 있다.

-------------------------------------------------------------------------------------

#### 파일 입출력( Java Application 관점 )
	Stream 이라는 연결통로를 통해 원본 데이터가 알맞은 인코딩 방식으로 전송된다.
...
#### File
	...
	디렉토리 생성, 해당 경로의 전체 파일 목록, 파일 삭제 등...
